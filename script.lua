return function(key)
    ...
    local function decompile(s)
        local url = string.format("https://oracle.mshq.dev/decompile?key=%s", key)
        ...
        elseif response.StatusCode == 400 then
            return "-- Update the decompiling script / Check decompiling options for errors"

        else
            return "-- Something went wrong when decompiling: " .. response.StatusCode
        end
    end
